...
function addAttributeDiv (AttributesGroupsDiv, _flatID, attrid, Groups) {
    ...
    if (searchForSign == '') { // Ak sa atribút nemá "zgrupovať", vypíš iba túto jednu skupinu
    ...
    GroupsFirstChild.addClass('active');

    // Výpis atribútov - prvky
    $('.attr-group').empty().append('<div class="active-attribute-image item"></div>'); // Thumbnail akvívneho atribútu
        for (var i = 0; i < AllFlats[_flatID][attrid].Names.length; i++) { // Generovanie miniatúr atribútov
            var item = document.createElement('div');
            $(item).addClass('item').addClass('attr-item').addClass(AllFlats[_flatID][attrid].Thumbnails[i]);
            if (i == (AllFlats[_flatID][attrid].ActiveItem-1)) { // ak je položka predvolená
            ...
            if (AllFlats[_flatID][attrid].Premium[i] == 'y') { // Ak je položka prémiová
    ...
    Groups.on('click touchstart', function(){
        AtributesArray = [];
        // Vytvorenie poľa s Idčkami "grouped" atribútov
        $.each(AllFlats[_flatID], function (i) {
            if (AllFlats[_flatID][i].Grouped == searchForSign) {
                AtributesArray.push(AllFlats[_flatID][i].id);
            }
        ...
        for (var i = 0; i < AllFlats[_flatID][AtributesArray[Index]].Names.length; i++) { // Generovanie miniatúr atribútov
            var item = document.createElement('div');
            $(item).addClass('item').addClass('attr-item').addClass(AllFlats[_flatID][AtributesArray[Index]].Thumbnails[i]);
            if (i == (AllFlats[_flatID][AtributesArray[Index]].ActiveItem-1)) { // ak je položka predvolená
                $(item).addClass('active');
            ...
            if (AllFlats[_flatID][AtributesArray[Index]].Premium[i] == 'y') { // Ak je položka prémiová
                $(item).addClass('premium');
            }
            $('.attr-group').append(item);
        }
