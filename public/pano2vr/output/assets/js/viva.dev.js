...
$.html5Loader({
  filesToLoad: 'assets/json/data.json',
  // this could be a JSON or simply a javascript object
  onBeforeLoad: function onBeforeLoad() {////////console.log('preloaer spustený');
  },
  onComplete: function onComplete() {////////console.log('načítané dáta');
  },
  onElementLoaded: function onElementLoaded(obj, elm) {////////console.log(obj);
    ////////console.log(elm);
  },
...
function getSubtitlesLink(lang) {
  subtitlesUrl = urlPrefix[lang] + '/api/building-tour-translations?api_token=' + apiToken[lang]; //////////console.log(subtitlesUrl);
}
...
function loadHousesData(housesUrl, lang) {
  if (housesData['int'] == undefined || housesData['int'] == null || housesData['int'] == '') {
    housesData['int'] = {};
    var int_housesUrl = 'https://int.baumit.com/api/buildings?api_token=AP7R6yAeBytyvc0OASz3RhLrAH2CsCGnNDWTeLBAbyW72sMPRCiw4JAKDTz0';
    var intHousesData = $.getJSON(int_housesUrl, function (data) {
      ...
      //houses_tour();;
      //add_screens();
      if (housesData[lang]['buildings'] != undefined) {
        $.each($('.swiper-slide'), function (index, data) {
          ...
          switch ($('.swiper-slide').eq(index).attr('data-url')) {
            ...
            default:
              if (housesData[lang]['buildings'][index - 1] != undefined) {
        ...
        if ($('.vp-logo')) {
          if (housesData[lang]['additional_content'][15] != '' && housesData[lang]['additional_content'][15] != null) {
            if (housesData[lang]['additional_content'][15]['media'] != '' && housesData[lang]['additional_content'][15]['media'] != null) {
              $('.vp-logo > img').attr('src', housesData[lang]['additional_content'][15]['media']); ////////console.log('logo : ' + housesData[lang]['additional_content'][15]['media']);
            } else {
              $('.vp-logo > img').attr('src', 'images/vivapark-logo.svg'); ////////console.log('Logo nie je ');
            }
          }
        } else {
  ...
  if (housesData[lang] == undefined || housesData[lang] == null || housesData[lang] == '' || lang == 'int') {
    housesData[lang] = {};
...
function playPauseMedia($val) {
  ...
  var patchName = pano.getNodeUserdata(pano.getCurrentNode()).title;
  ...
  if (pano.getMediaObject(patchName) != null) {
    ...
    if (videoDuration == 0 || videoDurationHalf == 0 // pano.getMediaObject(patchName).currentTime == 0
    ) {
        ////console.log('teraz nemôžem prehrať');
        // vid = document.getElementById("vid1"); 
        vid = document.getElementById(patchName); ////console.log(vid);
        ...
        videoDurationHalf = videoDuration / 2 - 0.20; ////////console.log('Môžem prehrať 2 videá naraz');
      ...
      if (pano.isPlaying(patchName) == true) {////console.log('Nemôžem prehrať 2 videá naraz');
        ...
        if (video_nacitane == true && $val == false) {} else {
...
function playMedia() {// ////////console.log('Play media');
  // play = true;
  // vid.ontimeupdate = function() {
  //     if (play == true || pano.isPlaying(patchName) == false) {
  //         if(this.currentTime >= videoDurationHalf ) {       
...
function showVivaIntro() {
  if (currentNode == 'node1' && pano.getVariableValue('floorplan_full') == false && pano.getVariableValue('take_tour') == false) {
  ...
  if (houseID !== 100) {
    ...
    if (data == 0) {
      ...
      if (tmpLength == 0) {
        ...
        $('.viva-house-info > div > .parameters').remove(); ////////console.log(housesData[lang]['buildings'][tmpHouseID]);

        if (housesData[lang]['buildings'][tmpHouseID] != null && housesData[lang]['buildings'][tmpHouseID] != undefined && housesData[lang]['buildings'][tmpHouseID] != 'undefined') {
          // Parametre domu
          if (housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'] != '' && housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'] != 'undefined' && housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'] != null) {
            ...
            $.each(housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'], function (index, value) {
              ...
              if (housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'] != '' && housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'] != 'undefined' && housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'] != null) {
                $.each(housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'], function (i, data) {
                  $('.viva-house-info > div > .parameters').append('<div class="parameter-title">' + housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'][i]['parameter_name'] + '<span> i  ' + '<div class="house-tooltip">' + '<div class="house-tooltip-arrow">' + '</div>' + '<div class="house-tooltip-inner">' + housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'][i]['parameter_tooltip'] + '</div>' + '</div>' + '</span>' + '</div>');
    ...
    $.each(housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'], function (index, value) {
      $.each(housesData[lang]['buildings'][tmpHouseID]['parameters']['groups'][index]['parameters'], function (i, data) {
        ...
        function animate_paramater() {
          ...
          tmp_count_param = tmp_count_param + 1; ////////console.log('teraz');
        }

        var myTimeout = setTimeout(animate_paramater, 50);
      });
    });
  }
}
...
function loadIntroData() {
  //alert('agdg');
  //////////console.log(lang);
  if (housesData[lang]['additional_content'] != undefined) {
...
function changePanoTitle(lang, housesData) {
  ...
  $('#next-house').on('click tap', function () {
    ...
    if (pano.getIsLoading() == false) {
      clearInterval(New_interval);
      var index = is_tour_nodes.indexOf(pano.getCurrentNode()); ////////console.log('index : ' + index);
    ...
    } else {////console.log('ešte nemôžem prepnúť');
    }
  });
  ...
  switch (node_id) {
    ...
    case '13':
      if (housesData[lang]) {
        title.text(housesData[lang]['buildings'][parseInt(node_id) - 1].house_nr); // $('.learn-more > span').html(housesData[lang]['buildings'][parseInt(node_id)-1].house_nr);
  ...
  if (housesData[lang]) {
    ...
    $.each($('.swiper-slide'), function (index, data) {
      ...
      switch ($('.swiper-slide').eq(index).attr('data-url')) {
        ...
        default:
          if (housesData[lang]['buildings'] != undefined && housesData[lang]['buildings'][index - 1] != undefined) {
            $('.swiper-slide').eq(index).find('.node-title').html(housesData[lang]['buildings'][index - 1].house_nr);
          }
...
var is_tour_nodes = ['node1', 'node24', 'node12', 'node26', 'node6', 'node22', 'node3', 'node18', 'node5', 'node20'];
...
function add_take_tour_button() {
  $('.viva-start').append('<div class="intro-take-tour">' + '<p>Take<span>Tour</span></p>' + '</div>');
...
pano.on('configloaded', function (Tooltips) {
  ...
  pano.on('varchanged_take_tour', function (audio) {
    ...
    function change_scene($active_scene) {
      var index = is_tour_nodes.indexOf($active_scene);
    ...
    function play_videos() {
      var active_scene = pano.getCurrentNode();
      var active_video = take_tour_data[active_scene].videos[0].id; //////////console.log('current scene : '+ active_scene + '| current video : ' + active_video );

      if (pano.getMediaObject(active_video) == null || pano.getMediaObject(active_video) == undefined || pano.getMediaObject(active_video) == 'undefined') {
        ...
        pano.getMediaObject(active_video).addEventListener('ended', function () {
          ...
          if (take_tour_data[active_scene].videos[1] != null && take_tour_data[active_scene].videos[1] != undefined && take_tour_data[active_scene].videos[1] != 'undefined') {
            active_video = take_tour_data[active_scene].videos[1].id; //////////console.log('current scene : '+ active_scene + '| current video : ' + active_video );
            ...
            if (pano.getMediaObject(active_video) == null || pano.getMediaObject(active_video) == undefined || pano.getMediaObject(active_video) == 'undefined') {
              ...
              pano.setMediaVisibility(active_video, true);
              pano.moveTo(take_tour_data[active_scene].videos[1].pan, take_tour_data[active_scene].videos[1].tilt, 84, 5); //pano.setVolume(active_video, 0.0);
    ...
    switch (pano.getVariableValue('take_tour')) {
      case true:
        $('.take-tour-button').addClass('playing'); // dočasné vypnutie hotspotov
        //pano.setVariableValue('hotspots', false);

        $('.viva-start').addClass('hidden');
        run_baumit_tour(); // preventívne skrytie video patchov
        // pano.setMediaVisibility('video_1', false);
        // pano.setMediaVisibility('video_2', false);
        // aktivácia šípok

        take_tour_navigation_arrows();
        play_videos(); //////////console.log('zapol som take tour');

        break;

      case false:
        $('#houses-info-container').remove();
        $('.take-tour-button').removeClass('playing'); //////////console.log('vypol som take tour');

        pano.setVariableValue('hotspots', true);

        if (pano.getMediaObject('video_1') != null && pano.getMediaObject('video_1') != undefined && pano.getMediaObject('video_1') != 'undefined') {
          //   pano.setMediaVisibility('video_1', false);
          pano.stopSound('video_1');
        }

        if (pano.getMediaObject('video_2') != null && pano.getMediaObject('video_2') != undefined && pano.getMediaObject('video_2') != 'undefined') {
          // pano.setMediaVisibility('video_2', false);
  ...
  pano.on('changenode', function nodechange(lang) {
    ...
    if (pano.getCurrentNode() == 'node1') {
      if (pano.getVariableValue('baumit_tour') == true) {
        pano.setVariableValue('baumit_tour', false);
        welcome_screen();
      }
    ...
    tmpTiles++; //addLoader('loading products data');
    ...
    if (currentNode == 'node24' || currentNode == 'node18' || currentNode == 'node20') {
      change_patches();
    }
...
function play_Animation() {
  setTimeout(function () {
    if (currentNode != 'node1' && // currentNode != 'node16' &&
    currentNode != 'node24' && currentNode != 'node26' && currentNode != 'node30' && currentNode != 'node31' && currentNode != 'node32' && currentNode != 'node33' && pano.getMediaObject(patchName) != null) {
      if (pano.getMediaObject(patchName).lastElementChild.attributes.src.nodeValue == null || pano.getMediaObject(patchName).lastElementChild.attributes.src.nodeValue == undefined || pano.getMediaObject(patchName) == null) {
        ...
        if (pano.getVariableValue('baumit_tour') == true && pano.getMediaObject(pano.getNodeUserdata(pano.getCurrentNode()).title).currentTime == 0) {
          playPauseMedia();
        }

        if (pano.getVariableValue('take_tour') == true && tilesready_count < 1) {
  ...
  pano.on('tilesready', function () {////console.log('Tiles sú ready');
  });
}
...
function welcome_screen() {
  ...
  var description;
  var more_info;
  var about_viva_park, visit_and_enjoy_tour, rules_of_healthy_living, play, virtual_tour;
  ...
  if (housesData[lang]['additional_content'] != undefined) {
    ...
    if (housesData[lang]['additional_content'][35] == null || housesData[lang]['additional_content'][35] == undefined || housesData[lang]['additional_content'][35] == '') {
    ...
    if (housesData[lang]['additional_content'][19] == null || housesData[lang]['additional_content'][19] == undefined || housesData[lang]['additional_content'][19] == '') {
      more_info = 'More info';
    } else {
      more_info = housesData[lang]['additional_content'][19]['title'];
    }

    if (housesData[lang]['additional_content'][47] == null || housesData[lang]['additional_content'][47] == undefined || housesData[lang]['additional_content'][47] == '') {
      play = 'Play';
    } else {
      play = housesData[lang]['additional_content'][47].title;
    }

    if (housesData[lang]['additional_content'][50] == null || housesData[lang]['additional_content'][50] == undefined || housesData[lang]['additional_content'][50] == '') {
      rules_of_healthy_living = '3  rules of healthy living';
    } else {
      rules_of_healthy_living = housesData[lang]['additional_content'][50].title;
    }

    if (housesData[lang]['additional_content'][52] == null || housesData[lang]['additional_content'][52] == undefined || housesData[lang]['additional_content'][52] == '') {
      visit_and_enjoy_tour = 'Visit and enjoy tour';
    } else {
      visit_and_enjoy_tour = housesData[lang]['additional_content'][52].title;
    }

    if (housesData[lang]['additional_content'][51] == null || housesData[lang]['additional_content'][51] == undefined || housesData[lang]['additional_content'][51] == '') {
      about_viva_park = 'About the VIVA Park';
    } else {
      about_viva_park = housesData[lang]['additional_content'][51].title;
    }

    if (housesData[lang]['additional_content'][9] == null || housesData[lang]['additional_content'][9] == undefined || housesData[lang]['additional_content'][9] == '') {
    ...
    $('#welcome').remove();
    $('body').append('<div id="welcome">' + '<div id="wrapper">' + '<div id="header">' + '<h1>' + title + '</h1>' + '<h2>' + subtitle + '</h2>' + '<p>' + description + '</p>' + '<div class="buttons">' + '<button id="play_tour">' + play + '</button>' + '<button id="more_info">' + more_info + '</button>' + '</div>' + '</div>' + '<div id="footer">' + '<div id="research" class="item">' + '<div class="thumbnail">' + '<img src="images/btn-1.jpg" />' + '</div>' + '<div class="text">' + '<h4>' + virtual_tour + '</h4>' + '</div>' + '</div>' + '<div id="rules" class="item">' + '<div class="thumbnail">' + '<img src="images/btn-2.jpg" />' + '</div>' + '<div class="text">' + '<h4>' + rules_of_healthy_living + '</h4>' + '</div>' + '</div>' + // '<div id="about-viva" class="item">'+
    //     '<div class="thumbnail">'+
    ...
    //     '</div>'+
    // '</div>'+
    '<div id="visit-tour" class="item">' + '<div class="thumbnail">' + '<img src="images/00_Free_tour_icon_f.jpg"/>' + '</div>' + '<div class="text">' + '<h4>' + visit_and_enjoy_tour + '</h4>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>');
    $('#more_info').unbind();
    ...
    $('#play_tour').on('click tap', function () {
      $('#play_tour').unbind();
      $('#welcome').fadeOut();
      pano.setVariableValue('hotspots', true); //pano.setVariableValue('houses_tour', true);

      pano.setVariableValue('baumit_tour', true);
      var node = pano.getCurrentNode();
      var active_scene = pano.getCurrentNode();
    ...
    $('#research').on('click tap', function () {
      $('#research').unbind(); // $('#welcome').fadeOut();
...
function loadSubtitlesData(subtitlesUrl, lang) {
  ...
  if (subtitlesData[lang] == undefined || subtitlesData[lang] == null || subtitlesData[lang] == '') {
    subtitlesData[lang] = {};
    var tmpHousesData = $.getJSON(subtitlesUrl, function (data) {
      ...
      generate_baumit_data(); ////////console.log(subtitlesData);
    });
  }
}
...
function houses_screen() {
  ...
  if (housesData[lang] == 'undefined' || housesData[lang] == null) {
    return;
  } // Hosue #1


  if (housesData[lang]['additional_content'][36] == null || housesData[lang]['additional_content'][36] == undefined || housesData[lang]['additional_content'][36] == '') {
    house_1 = 'Home';
  } else {
  ...
  if (housesData[lang]['additional_content'][37] == null || housesData[lang]['additional_content'][37] == undefined || housesData[lang]['additional_content'][37] == '') {
  ...
  if (housesData[lang]['additional_content'][38] == null || housesData[lang]['additional_content'][38] == undefined || housesData[lang]['additional_content'][38] == '') {
  ...
  if (housesData[lang]['additional_content'][39] == null || housesData[lang]['additional_content'][39] == undefined || housesData[lang]['additional_content'][39] == '') {
  ...
  if (housesData[lang]['additional_content'][40] == null || housesData[lang]['additional_content'][40] == undefined || housesData[lang]['additional_content'][40] == '') {
  ...
  if (housesData[lang]['additional_content'][41] == null || housesData[lang]['additional_content'][41] == undefined || housesData[lang]['additional_content'][41] == '') {
  ...
  function open_next_node() {
    for (var index = 0; index < is_tour_nodes.length; index++) {
      var i = index + 1;
      var node = pano.getCurrentNode(); ////////console.log(node);

      if (node == is_tour_nodes[index]) {
        if (index == is_tour_nodes.length) {
          pano.openNext('{' + is_tour_nodes[0] + '}');
          return;
        } else {
  ...
  var learn_more_about;

  if (housesData[lang]['additional_content'][54] == null || housesData[lang]['additional_content'][54] == undefined || housesData[lang]['additional_content'][54] == '') {
    learn_more_about = 'Learn more';
  } else {
    learn_more_about = housesData[lang]['additional_content'][54].title;
  }

  $('body').append('<div id="houses-info-container">' + '<div class="houses-header">' + '<div class="buttons">' + '<img class="play hidden" src="assets/icons/play-houses.svg"/>' + '<img class="pause" src="assets/icons/pause-houses.svg"/>' + '<img class="toggle" src="assets/icons/toggle-houses.svg"/>' + '<button id="next-house">Next house</button>' + '<button id="learn-more" class="learn-more">' + learn_more_about + '&nbsp;<span>' + pano.getNodeUserdata(pano.getCurrentNode()).source + '</span></button>' + '</div>' + '<div class="subtitles">' + '<p>' + current_subtitle + '</p>' + '</div>' + '</div>' + '<div class="houses-footer">' + '<div id="house_1" class="item">' + '<img class="icon" src="images/house-default.png"/>' + '<p>' + house_1 + '</p>' + '</div>' + '<div id="house_2" class="item">' + '<img class="icon" src="images/house-default.png"/>' + // '<p>' + house_2_title + '<br>' + house_2_content + '</p>'+
  '<p>' + house_2_title + '</p>' + '</div>' + '<div id="house_3" class="item">' + '<img class="icon" src="images/house-massive.png"/>' + // '<p>' + house_3_title + '<br>' + house_3_content + '</p>'+
  '<p>' + house_3_title + '</p>' + '</div>' + '<div id="house_4"  class="item">' + '<img class="icon" src="images/house-wooden.png"/>' + // '<p>' + house_4_title + '<br>' + house_4_content + '</p>'+
  '<p>' + house_4_title + '</p>' + '</div>' + '<div id="house_5" class="item">' + '<img class="icon" src="images/house-aerated.png"/>' + // '<p>' + house_5_title + '<br>' + house_5_content + '</p>'+
  '<p>' + house_5_title + '</p>' + '</div>' + '<div id="house_6" class="item">' + '<img class="icon" src="images/house-brick.png"/>' + // '<p>' + house_6_title + '<br>' + house_6_content + '</p>'+
  '<p>' + house_6_title + '</p>' + '</div>' + '</div>' + '</div>');
  changePanoTitle(lang, housesData);
  $('.play').on('click tap', function () {
    $(this).addClass('hidden');
    $('.pause').removeClass('hidden'); //pano.setVariableValue('take_tour', true);
  });
  $('.pause').on('click tap', function () {
    $(this).addClass('hidden');
    $('.play').removeClass('hidden'); //pano.setVariableValue('take_tour', false);
  });
...
function generate_baumit_data() {
  ...
  $.each(baumit_object, function (index, node) {
    $.each(node, function (number, sequence) {
      $.each(sequence, function (_int, data) {
        if (data.languages[lang] == 'undefined' || data.languages[lang] == undefined || data.languages[lang] == null) {
          data.languages[lang] = {};
          data.languages[lang].subtitles = '';
        }

        if (data.languages[lang].subtitles == '') {
          for (var _index5 = 0; _index5 < data.subtitles_fields.length; _index5++) {
            data.languages[lang].subtitles = data.languages[lang].subtitles + subtitlesData[lang][data.subtitles_fields[_index5]];

            if (_index5 < data.subtitles_fields.length - 1) {
...
function run_baumit_tour() {
  ...
  function mark_active_house() {
    ...
    select_active_house(); ////////console.log('markujem ... ' + current_house);
  }
    ...
    for (var index = 0; index < is_tour_nodes.length; index++) {
      if (is_tour_nodes[index] == node) {
        if (index == is_tour_nodes.length) {
          ...
          if (is_tour_nodes[i] == null || is_tour_nodes[i] == 'undefined') {
  ...
  mark_active_house(); //pano.setVolume('_main', 0.0);
  ...
  function change_data() {
    ...
    audio1 = '';
    var max_seq = 3;
    var current_node = pano.getCurrentNode();
    var current_lang = pano.getVariableValue('lang');
    var i = 1;
    ...
    function pusti_zvuk() {
      ...
      setTimeout(function () {
        ...
        if (baumit_object[current_node].sequences[i] != null && baumit_object[current_node].sequences[i] != 'undefined') {
          $('.subtitles').html(baumit_object[pano.getCurrentNode()].sequences[i].languages[pano.getVariableValue('lang')].subtitles);
          audio1 = new Audio(subtitlesData[current_lang][baumit_object[current_node].sequences[i].audio_fields[0]]); ////////console.log('index je : ' + i);

          setTimeout(function () {
            if (pano.isPlaying('video_' + i)) {
          ...
          if (pano.getVariableValue('paused_baumit_tour') == false) {
              ...
              pano.getMediaObject(a_video).addEventListener('play', function () {
                ...
                function checkTime() {
                  if (pano.getCurrentNode() == 'node24') {
                    ...
                    if (timeObject[tmpcounter] != undefined) {
                      if (pano.getMediaObject(a_video).currentTime >= parseFloat(timeObject[tmpcounter]['time'].replace(':', '.')) * 100) {
                        ////console.log('zvyšujem counter');
                        if (counter == Object.keys(timeObject).length + 1) {
                          //            //console.log('cieľ');
                          clearInterval(interval); //return
                        } else {////console.log('cieľ nie je ');
                          }

                        counter++;
                      } else {////console.log('nerovná sa :: ' + pano.getMediaObject(a_video).currentTime*100 +' | ' + parseFloat(timeObject[counter]['time'].replace(':', '.'))*100 );
                      }
                    } else {//console.log('čo ja viem');
                      }
          ...
          a_video = 'video_' + i; //playPauseMedia();
          ...
          pano.getMediaObject(a_video).addEventListener('ended', function () {
            ...
            if (pano.getMediaObject('video_' + i) != null && pano.getMediaObject('video_' + i) != 'undefined') {
          ...
          audio1.onended = function () {// //console.log('Video '+i+' skončilo');
            // if (
            //     pano.getMediaObject('video_'+i) != null &&
            //     pano.getMediaObject('video_'+i) != 'undefined'
            // ) {
    ...
    pusti_zvuk(); //     ////////console.log('index je : ' + i);
    //     ////////console.log(subtitlesData[current_lang][baumit_object[current_node].sequences[i].audio_fields[0]]);
  ...
  pano.on('changenode', function mark() {
    ...
    if (pano.getVariableValue('baumit_tour')) {
      var _node = pano.getCurrentNode();

      change_data(); //pano.moveTo(take_tour_data[node].videos[0].pan, take_tour_data[node].videos[0].tilt, 84, 5);

      pano.setPanTiltFov(take_tour_data[_node].videos[0].pan, take_tour_data[_node].videos[0].tilt, take_tour_data[_node].videos[0].fov);
    } else {
      audio1.pause();
    }
  });
  ...
  $('#houses-info-container').remove();
  $('body').append('<div id="houses-info-container" style="display: none;">' + '<div class="houses-header">' + '<div class="buttons">' + '<img class="play hidden" src="assets/icons/play-houses.svg"/>' + '<img class="pause" src="assets/icons/pause-houses.svg"/>' + '<img class="toggle" src="assets/icons/toggle-houses.svg"/>' + '<button id="next-house">Next house</button>' + '<button id="learn-more" class="learn-more">' + learn_more_about + '&nbsp;<span>' + pano.getNodeUserdata(pano.getCurrentNode()).source + '</span></button>' + '</div>' + '<div class="subtitles">' + '<p>' + baumit_object[pano.getCurrentNode()].sequences[1].languages[lang].subtitles + '</p>' + '</div>' + '</div>' + '<div class="houses-footer">' + '<div id="house_1" class="item">' + '<img class="icon" src="images/house-default.png"/>' + '<p>' + house_1 + '</p>' + '</div>' + '<div id="house_2" class="item">' + '<img class="icon" src="images/house-default.png"/>' + // '<p>' + house_2_title + '<br>' + house_2_content + '</p>'+
  '<p>' + house_2_title + '</p>' + '</div>' + '<div id="house_3" class="item">' + '<img class="icon" src="images/house-massive.png"/>' + // '<p>' + house_3_title + '<br>' + house_3_content + '</p>'+
  '<p>' + house_3_title + '</p>' + '</div>' + '<div id="house_4"  class="item">' + '<img class="icon" src="images/house-wooden.png"/>' + // '<p>' + house_4_title + '<br>' + house_4_content + '</p>'+
  '<p>' + house_4_title + '</p>' + '</div>' + '<div id="house_5" class="item">' + '<img class="icon" src="images/house-aerated.png"/>' + // '<p>' + house_5_title + '<br>' + house_5_content + '</p>'+
  '<p>' + house_5_title + '</p>' + '</div>' + '<div id="house_6" class="item">' + '<img class="icon" src="images/house-brick.png"/>' + // '<p>' + house_6_title + '<br>' + house_6_content + '</p>'+
  '<p>' + house_6_title + '</p>' + '</div>' + '</div>' + '</div>');
  changePanoTitle(lang, housesData); // defaultné vyznačenie aktívneho domu
  ...
  pano.on('varchanged_paused_baumit_tour', function () {
    switch (pano.getVariableValue('paused_baumit_tour')) {
      ...
      case false:
        ...
        $('.pause').removeClass('hidden');
        $('.play').addClass('hidden'); // switch (pano.getCurrentNode()) {
        //     case 'node1' : 
        //         if (
    ...
    if ($('.houses-footer').hasClass('hidden') == true) {
  ...
  $('.play').on('click tap', function () {
    ...
    $(this).addClass('hidden');
    $('.pause').removeClass('hidden');
    pano.setVariableValue('paused_baumit_tour', false); //audio1.play();    
  });
  $('.pause').on('click tap', function () {
    $(this).addClass('hidden');
    $('.play').removeClass('hidden'); //pano.setVariableValue('baumit_tour', false);  

    pano.setVariableValue('paused_baumit_tour', true);
    audio1.pause();

    if (pano.isPlaying('video_1') == true) {
      pano.playPauseSound('video_1');
      a_video = 'video_1';
    }

    if (pano.isPlaying('video_2') == true) {
  ...
  pano.on('varchanged_baumit_tour', function () {
    if (!pano.getVariableValue('baumit_tour')) {
      ...
      ////console.log(a_video);
      a_video = 'video_1';
      pano.setMediaVisibility(a_video, 1); //audio1.volume = 0.0;
      ...
      if (pano.getMediaObject(pano.getNodeUserdata(pano.getCurrentNode()).title) != null) {
        if (pano.getMediaObject(pano.getNodeUserdata(pano.getCurrentNode()).title).currentTime == 0 || pano.getMediaObject(pano.getNodeUserdata(pano.getCurrentNode()).title).currentTime == pano.getMediaObject(pano.getNodeUserdata(pano.getCurrentNode()).title).duration) {
          videoDuration = 0;
          videoDurationHalf = 0;
          playPauseMedia();
        }
...
function download_data($value) {
  ...
  if (housesData[lang]['additional_content'][$value] == null || housesData[lang]['additional_content'][$value] == undefined || housesData[lang]['additional_content'][$value] == 'undefined') {
    ...
    switch ($value) {
      case '34':
        if (housesData[lang]['additional_content'][$value]['media'] == 'undefined' || housesData[lang]['additional_content'][$value]['media'] == null) {
          url = housesData['int']['additional_content'][$value]['media'];
        } else {
      ...
      default:
        if (housesData[lang]['additional_content'][$value]['link_url'] == 'undefined' || housesData[lang]['additional_content'][$value]['link_url'] == null) {
          url = housesData['int']['additional_content'][$value]['link_url'];
        } else {
...
pano.on('varchanged_baumit_tour', function () {
  switch (pano.getVariableValue('baumit_tour')) {
    case true:
      $('#houses-info-container').fadeIn(); ////////console.log('zapla sa baumit_tour');

      $('.take-tour-button').addClass('playing');
      var node = pano.getCurrentNode(); //pano.moveTo(take_tour_data[node].videos[0].pan, take_tour_data[node].videos[0].tilt, 84, 5);
    ...
    case false:
      ...
      for (var index = 0; index < i; index++) {
        if (pano.getMediaObject('video_' + index) != null && pano.getMediaObject('video_' + index) != 'undefined') {
...
pano.on('changenode', function () {
  var inter_scene = pano.getNodesWithTag('int');

  if (jQuery.inArray(pano.getCurrentNode(), inter_scene) !== -1) {
