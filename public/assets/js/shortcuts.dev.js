...
function PlayTour() {
  progressBar.removeClass('anim');
  var tmp_node_id = pano.getCurrentNode();
  var tmp_id = pano.getNodeUserdata(pano.getCurrentNode()).source;
  var my_array = allArrays['panoArray' + sliderTags[0]];
  var current_index = my_array.indexOf(tmp_node_id) + 1;
...
pano.on("configloaded", function () {
  for (var _index = 0; _index < sliderTags.length; _index++) {
    allArrays['panoArray' + sliderTags[_index]] = pano.getNodesWithTag(sliderTags[_index]);
    $('.slider-' + sliderTags[_index]).attr('id', sliderTags[_index]);
  }
  ...
  $('.slider-bg').append('<div class="swiper">' + '<div class="swiper-wrapper"></div>' + '<div class="swiper-pagination"></div>' + '<div class="swiper-button-prev"></div>' + '<div class="swiper-button-next"></div>' + '</div>' + '</div>');
  var copyright = pano.getNodeUserdata(pano.getCurrentNode()).source;
  ...
  function open_swiper_slide() {
    ...
    $('.swiper-button-next').on('click tap', function () {
      ...
      var tmp_id = pano.getNodeUserdata(pano.getCurrentNode()).source;
      var my_array = allArrays['panoArray' + sliderTags[0]];
      var current_index = my_array.indexOf(tmp_node_id);
    ...
    $('.swiper-button-prev').on('click tap', function () {
      var tmp_node_id = pano.getCurrentNode();
      var tmp_id = pano.getNodeUserdata(pano.getCurrentNode()).copyright;
      var my_array = allArrays['panoArray' + sliderTags[0]];
      var current_index = my_array.indexOf(tmp_node_id);
  ...
  function add_slider($id) {
    ...
    if ($id != 'vyberovnik') {
      ...
      for (var _index2 = 0; _index2 < allArrays['panoArray' + id].length; _index2++) {
        //console.log(allArrays['panoArray'+id][index]);
        swiper.appendSlide(['<div class="swiper-slide" data-url="' + allArrays['panoArray' + id][_index2] + '"><img data-url="' + allArrays['panoArray' + id][_index2] + '" src="images/node_image_' + allArrays['panoArray' + id][_index2] + '.png"><div class="node-title">' + pano.getNodeUserdata(allArrays['panoArray' + id][_index2]).comment + '</div></div>']);
      }
  ...
  $('.byt.f-cat > .tour').on('click tap', function () {
    ...
    var id = $(this).parent().attr('id').replace('byt-', '');
    add_slider(id);
    swiper.update();
    swiper.slideTo(0);
