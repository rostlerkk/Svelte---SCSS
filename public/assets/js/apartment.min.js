...
function filterCategory(cat1, cat2, cat3) {
  checkFilterHeight(); // reset results list
...
$('.filter-cat-results .f-cat').addClass('active'); // call the filtering function when selects are changed
...
function selectLastVisited() {
  ...
  $('.filter-cat-results > .byt > .last-visited-txt').remove(); //$('#'+LastVisitedOrientation).addClass('last-visited');

  $('.filter-cat-results > .last-visited').append('<div class="last-visited-txt">' + textLastVisitedEN + '</div>'); // switch (pano.getVariableValue('lang')) {
  //     case 'sk':
...
pano.on('configloaded', function changeSelectDesign() {
  selectLastVisited();
  $('select').niceSelect(); // Naplnenie poľa so všetkými bytmi
...
pano.on('changenode', function apartmentsChangeNode() {
  ...
  checkLastNode();
  selectLastVisited(); // zapínanie a vypínanie ShortCuts ikony
...
pano.addListener('varchanged_footer_apartments', function footerApartments() {
  ...
  switch (pano.getVariableValue('footer_apartments')) {
    case true:
      ...
      addClass(['.item.apartment'], 'active'); //ShowElement(['.apartments-side']);
...
pano.addListener('varchanged_3D_model', function model3D() {
  ...
  switch (pano.getVariableValue('3D_model')) {
    case true:
      ...
      tooltipModelFristTime++; // if ($('#3DObject').attr('src') == '' ) {
...
$('#b-307.byt-wrapper').on('click tap', function () {
  AllFlatColorChange($(this).attr('id')); // zmeň pozíciu
...
$('#b-306.byt-wrapper').on('click tap', function () {
  ...
  pano.showOnePolyHotspot($(this).attr('id')); // zmeň pozíciu
...
$('#305-tour').on('click tap', function () {
  openCorrectPano('305', 'kupelnaObklad', 'node7', 'node5', 'node6'); //pano.openNext('{node1}');
});
...
JumptoLastVisitedFlat.on('click touchstart', function ScrollToElement() {
  ...
  var position = scrollTo.offset().top - (container.offset().top + 139) + container.scrollTop();
  container.animate({
    scrollTop: position
  }, 200);
});
