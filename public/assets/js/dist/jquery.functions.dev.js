...
function AddReadMore(readMoreTxt, readLessTxt) {
  ...
  var carLmt = 200; // Text to show when text is collapsed
...
function najdiAktualnyByt() {
  ...
  } else {//console.log('Aktuálny byt je '+bytID);
  }
}
...
function readFlats(response, Flats) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    var AddItem = function AddItem() {
      ...
      Flats[_flatID][_flatAttrID].target = _flatAttrID; // Načítanie názvov atribútov

      Flats[_flatID][_flatAttrID].names = {
        sk: Ids[_flatAttrID]._attrNames.sk,
        en: Ids[_flatAttrID]._attrNames.en
      };
      Flats[_flatID][_flatAttrID].targetUrl = Ids[_flatAttrID]._attrTarget; // Načítanie popisov atribútov
...
function readHotspots(response, Hotspots) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    _hotspotTarget = lines[i].split(";")[1];

    if (_panoID == '') {} //ak byt existuje, zapíše do neho hodnoty
...
function nacitajId() {
  ...
  $.ajax({
    ...
    success: function importIds(response) {
      ...
      readIds(response, Ids); // console.log('Načítal som  1. CSV súbor "ids.csv"...');
...
function nacitajByty() {
  $.ajax({
    ...
    success: function importFlats(response) {
      ...
      readFlats(response, Flats); // console.log('Načítal som 2. CSV súbor "import.csv"...');
    ...
    complete: function complete() {
      console.log('2. CSV súbor načítaný korektne - BYTY OK.'); //console.log(Flats[bytID]);
      //console.log(Flats);
    }
  });
}
...
function nacitajHotspoty() {
  $.ajax({
    ...
    success: function importHotspots(response) {
      ...
      readHotspots(response, Hotspots); //console.log('Načítal som 2. CSV súbor "import.csv"...');
      //nacitajPatche();
    ...
    complete: function complete() {
      console.log('3. CSV súbor načítaný korektne - Hotspoty OK.'); //console.log(Flats[bytID]);
      //console.log(Flats);
...
function UpdateCurrentPatches() {
  ...
  if (patches[activeNode] == undefined || patches[activeNode] == {}) {
    ...
    if (pocetPatchov > 0 || Flats[bytID] != undefined) {
      ...
      for (i = 0; i < patchesNames.length; i++) {
        ...
        aTitle = patchesNames[i]; //console.log(patchesNames[i]);
...
function attributesReady() {
  var hts = $('.hts');
  var numItems = hts.length; //console.log('Počet hotspotov je '+numItems);

  counter = numItems;
  activeItem = pano.getCurrentNode(); //console.log(activeItem);

  var keys = Object.keys(Hotspots[activeItem]); //console.log(keys);
...
function addItemToCart(Flats) {
  ...
  $.each(Flats[bytID], function (index, item) {
    ...
    function shortText(item) {
      ...
      if (txt.length > CartDescLimit) {
        txt = txt.substring(0, CartDescLimit) + ' ...'; //console.log(txt);
    ...
    if (selectByPremium == true) {
      ...
      if (Flats[bytID][index].premium[activeItem] == 'premium') {
        var loopedElement = '<div class="attributes-item opened">' + '<div>' + '<div class="att-title premium" id=' + index + '>' + item.title[0] + '<span class="closed"></span></div>' + '<div class="values-group">' + '<div class="value">' + '<div class="left">' + '<div class="thumb ' + item.thumbnails[activeItem] + '"></div>' + '<div class="parameters">' + '<div class="param-title">' + item.names[lang][activeItem] + '</div>' + '<div class="param-description">' + shortText(item) + '</div>' + '</div>' + '</div>' + '<div class="right">' + '<div class="param-price">' + item.prices[activeItem] + '</div>' + '</div>' + '</div>' + '</div>' + '</div>';
        '</div>';
        attrGroup.append(loopedElement);
      } else {
        var loopedElement = '<div class="attributes-item opened">' + '<div>' + '<div class="att-title" id=' + index + '>' + item.title[0] + '<span class="closed"></span></div>' + '<div class="values-group">' + '<div class="value">' + '<div class="left">' + '<div class="thumb ' + item.thumbnails[activeItem] + '"></div>' + '<div class="parameters">' + '<div class="param-title">' + item.names[lang][activeItem] + '</div>' + '<div class="param-description">' + shortText(item) + '</div>' + '</div>' + '</div>' + '<div class="right">' + '<div class="param-price">' + item.prices[activeItem] + '</div>' + '</div>' + '</div>' + '</div>' + '</div>';
        '</div>';
        attrGroup.append(loopedElement);
      }
    } else {
  ...
  var attTitle = $('.att-title'); // Rozbaliť/skryť div values-group a zmena ikony + a - vedľa názvu skupiny atribútov
...
var selectByPremium = true; // false - riadi sa podľa default hodnoty, true - riadi sa poďla premium označenia hodnoty atribútu
