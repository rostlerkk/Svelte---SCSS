...
pano.on('changenode', function nodechange() {
  ...
  if (patchName != 'Exterier_00') {
    ...
    pano.on('tilesready', function addPatch() {
      ...
      if (pano.getCurrentNode() != 'node26') {
        if (pano.getMediaObject(patchName).lastElementChild.attributes.src.nodeValue == null || pano.getMediaObject(patchName).lastElementChild.attributes.src.nodeValue == undefined) {
...
function nacitajData(productName) {
  if (productNameSave == productName && productLang == pano.getVariableValue('lang')) {
    $('.hotspot-loader').remove();
    console.log('pozor nie je zmena');
    pano.setVariableValue('infopanel', true);
  } else {
    if (productName == '' || productName == undefined || productName == null) {} else {
      $('.info-v1 > .head > .content > h2').empty();
      $('.info-v1 > .head > .content > a').remove();
      $('.content > ul').empty();
      $('.info-v1 > .body > p').empty();
      $('.info-v1 > .head > .baumit-img > img').remove();
      console.log('je je zmena'); //console.log(productNameSave);

      pano.setVariableValue('infopanel', true); //$('#container').append(loader);

      lang = pano.getVariableValue('lang'); //searchFor = productName;
      ...
      $('.info-v1 > .head > .baumit-img').append("<img class='baumit-img' src='" + imgSrc + "'/>"); //$('.info-v1 > .head > .baumit-img > img').attr('src', imgSrc);
...
function checkHotspots() {
  ...
  for (i = 0; i < hotspotsArray.length; i++) {
    ...
    if (hotspotsArray[i].className.indexOf('viva-tooltip') > -1) {
      tmpProductId = hotspotsArray[i].textContent.substring(hotspotsArray[i].textContent.indexOf("|") + 1); //console.log(tmpProductId);
    ...
    if (tmpProductId != '') {
      ...
      if (dataModel[pano.getVariableValue('lang')][tmpSearchingID] == undefined || dataModel[pano.getVariableValue('lang')][tmpSearchingID] == null || dataModel[pano.getVariableValue('lang')][tmpSearchingID] == '') {} else {
        console.log(tmpSearchingID); //$('.viva-tooltip[data-id="'+tmpProductId+'"] > div > div').text(dataModel[tmpLang][tmpSearchingID].name);
...
function houseInfo(data) {
  ...
  if (houseID !== data) {
    ...
    if (dataModel[lang][houseID] != null || dataModel[lang][houseID] != undefined) {
      // ak ide o House 0
      if (dataModel[lang][houseID]['House No.'] == '0') {
      ...
      if (dataModel[lang][houseID]['Media'] != '') {
      ...
      if (dataModel[lang][houseID]['Link'] != '') {
        if (moreInfoButton[pano.getVariableValue('lang')] == undefined || moreInfoButton[pano.getVariableValue('lang')] == null || moreInfoButton[pano.getVariableValue('lang')] == 'undefined') {
...
function createDataModel() {
  ...
  if (dataModel[lang] == undefined || dataModel[lang] == null || dataModel[lang] == {}) {
    dataModel[lang] = {};
    ...
    $.getJSON(url, function (data) {
      $.each(data, function (key, val) {
        ...
        if (tmpID == undefined || tmpID == null || tmpID == '') {
          if (data[key]['House No.'] != '' || data[key]['House No.'] != undefined || data[key]['House No.'] != null) {
          ...
          var _dataModel$lang$tmpID;

          dataModel[lang][tmpID] = {};
          dataModel[lang][tmpID] = (_dataModel$lang$tmpID = {
            'name': data[key]['name'],
            'epim_name': data[key]['epim_name']
          }, _defineProperty(_dataModel$lang$tmpID, "epim_name", data[key]['epim_name']), _defineProperty(_dataModel$lang$tmpID, 'additional_name', data[key]['additional_name']), _defineProperty(_dataModel$lang$tmpID, 'description', data[key]['description']), _defineProperty(_dataModel$lang$tmpID, 'product_benefit_1', data[key]['product_benefit_1']), _defineProperty(_dataModel$lang$tmpID, 'product_benefit_2', data[key]['product_benefit_2']), _defineProperty(_dataModel$lang$tmpID, 'product_benefit_3', data[key]['product_benefit_3']), _defineProperty(_dataModel$lang$tmpID, "product_benefit_1", data[key]['product_benefit_1']), _defineProperty(_dataModel$lang$tmpID, 'image', data[key]['image']), _defineProperty(_dataModel$lang$tmpID, 'details_url', data[key]['details_url']), _dataModel$lang$tmpID);
        }
      });
      ...
      if (pano.getVariableValue('blurred') == false) {
        $('.ggskin_external').css({
          'visibility': 'visible'
        });
      }
