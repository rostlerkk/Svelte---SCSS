...
function LastVisitedFlat(LastVisited) {
  // var byt = $('.byt');
  hlpL = pano.getNodeUserdata(pano.getLastVisitedNode()).copyright;

  if (hlpL != 'vyberovnik') {
    LastVisited = hlpL; // console.log('Zmeni som LastVisited na : '+LastVisited);
  } else {} //console.log(hlpL+ ' | ' +LastVisited);


  byt.find('.last-visited').remove();
  $('#' + LastVisited).append(TextLastVisited); //console.log('Po spustení funkcie LastVisitedFlat je premenná hlpL :'+hlpL);
...
function FindActivePano() {
  ...
  } else {} // LastVisitedFlat(UserData, LastVisited, byt);
  ...
  switch (pano.getVariableValue('footer_apartments')) {
    case true:
      ...
      if (pano.getCurrentNode() != defaultOrientation) {
        pano.openNext('{' + defaultOrientation + '}');
      }
...
var filterActive;
var AllFlat = []; // Naplnenie poľa so všetkými bytmi
...
function filterCategory(cat1, cat2, cat3) {
  // reset results list
  $('.filter-cat-results .f-cat').removeClass('active'); // the filtering in action for all criteria
  ...
  $(selector).addClass('active'); // reset active filter

  filterActive = cat1;
} // start by showing all items


$('.filtering .f-cat').addClass('active'); // call the filtering function when selects are changed

$('.filtering select').change(function () {
  filterCategory($('.filtering select.cat1').val(), $('.filtering select.cat2').val(), $('.filtering select.cat3').val()); // vypnutie farieb všetkých polygon hotspotov

  AllFlatColorChange(); // zapnutie farieb všetkých zvolených hotspotov

  SelectedFlatColorChange(); // LastVisitedFlat(UserData, LastVisited, byt);
...
pano.on('configloaded', function () {
  ...
  PanoWithTagOrientation = pano.getNodesWithTag('orientacia'); //LastVisited = (pano.getNodeUserdata(pano.getCurrentNode())).copyright;
...
defaultOrientation = 'node13'; // Zisťovanie posledne otvorenej svetovej strany
...
pano.on('changenode', function () {
  ...
  AllFlatColorChange();
  SelectedFlatColorChange(); // Skrytie ikon ak sa jedná o panorámy : 
...
var hlpIntro = 1; //console.log(hlpIntro);
...
pano.addListener('varchanged_3D_model', function () {
  switch (pano.getVariableValue('3D_model')) {
    case true:
      $('#3DObject').attr('src', 'object2vr/output/loop.html');
      $('.model-blur').addClass('active'); // Kontrola, či sa ná zobraziť INTRO SCREEN
      ...
      hlpIntro++; // Kontrola, či sa má zobraziť ToolTip

      if (firstTimeModel == 0) {
        $('.tooltip.model').addClass('active');
        setTimeout(function () {
