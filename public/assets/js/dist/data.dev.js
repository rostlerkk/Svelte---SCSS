...
function readCSVFile(response, AllFlats) {
  ...
  for (var i = 1; i < lines.length; i++) {
    var AddElements = function AddElements() {
      ...
      AllFlats[_flatID][_attributeID].Names = AllFlats[_flatID][_attributeID].Names.toString().replace('\r', '').split('|'); //Popisy
      ...
      DescCount = AllFlats[_flatID][_attributeID].Descriptions.length; //Ceny
      ...
      PriceCount = AllFlats[_flatID][_attributeID].Prices.length; //Miniatúry
      ...
      ThumbCount = AllFlats[_flatID][_attributeID].Thumbnails.length; //Zoskupený atribút

      AllFlats[_flatID][_attributeID].Grouped = '';
      AllFlats[_flatID][_attributeID].Grouped = _attributesGrouped; //Prémiový atribút
      ...
      ThumbCount = AllFlats[_flatID][_attributeID].Premium.length; //Aktívna položka
    ...
    if (_flatID == '') {//console.log('Prázdny záznam');
