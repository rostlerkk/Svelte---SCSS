...
function Media6x(variable_id, media_1, media_2, media_3, media_4, media_5, media_6) {
  switch (pano.getVariableValue(variable_id)) {
    case 1:
      hMedia([media_1, media_2, media_3, media_4, media_5, media_6]); //console.log('Skryl som ' + media_1 + ' a ' + media_2);

      break;

    case 2:
      hMedia([media_1, media_2, media_3, media_4, media_5, media_6]);
      sMedia([media_1, media_2, media_3]); //console.log('Zobrazil som ' + media_1 + ' a skryl som ' + media_2);

      break;

    case 3:
      hMedia([media_1, media_2, media_3, media_4, media_5, media_6]);
      sMedia([media_4, media_5, media_6]); //console.log('Zobrazil som ' + media_2 + ' a skryl som ' + media_1);
...
pano.on('changenode', function () {
  ...
  switch (pano.getCurrentNode()) {
    ...
    case 'node1':
      ...
      Media2x('attribute_B', 'Vypln_heraklit', 'Vypln_rastre'); // Zatvorenie videa "ADELKA" po skončení

      pano.getMediaObject('Adelka').addEventListener('ended', function () {
        pano.setMediaVisibility('Adelka', 0, 250);
        variableTrue(['hotspots']);
        pano.playPauseSound("koma", "1");
        /* pustí zvuk na pozadí */
...
pano.addListener('varchanged_active_attr_group', function () {
  ...
  switch (pano.getVariableValue('active_attr_group')) {
    case 1:
      // Zvolená prvá skupiny atribútov (color)
    ...
    case 2:
      // Zvolená druhá skupiny atribútov (material)
      removeClass(['.group'], 'active');
...
pano.addListener('varchanged_attributes_side', function () {
  switch (pano.getVariableValue('attributes_side')) {
    case true:
      position = $('.attributes-side').outerHeight() - 258; //console.log(position);
...
var position = $('.active-attribute-image').offset(); //console.log(position);
